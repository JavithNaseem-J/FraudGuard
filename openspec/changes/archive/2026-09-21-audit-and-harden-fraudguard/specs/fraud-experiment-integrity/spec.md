...
The system SHALL compare a simple baseline and candidate models using a metric
appropriate to imbalanced fraud ranking, and SHALL apply imbalance handling
without resampling validation or test observations.

#### Scenario: Candidate selection
- **WHEN** model selection completes
- **THEN** the selected model is supported by training-only cross-validation average precision and comparison with a recorded baseline

### Requirement: Training-only threshold selection
The decision threshold SHALL be selected from training-only out-of-fold scores,
with its objective and value stored as model metadata. The final test set SHALL
not influence the threshold.

#### Scenario: Select operating threshold
- **WHEN** a model family and hyperparameters have been selected
- **THEN** the system derives the F1-maximizing threshold from out-of-fold training probabilities and records it before final test evaluation
...
Brier score, confusion matrix, support, prevalence, and threshold alongside a
baseline comparison.

#### Scenario: Evaluate decision policy
- **WHEN** the evaluation stage runs with a compatible fitted artifact
- **THEN** reported classification metrics and the confusion matrix use the stored threshold rather than the estimator's default class prediction
