...
import { saveScoredBatch } from '@/services/dashboardStore';
...
export const ScoreTransactionsPage: React.FC = () => {
  ...
  const handleRunFraudDetection = async () => {
    ...
    try {
      ...
      saveScoredBatch(summary, results, apiResp);
    } catch (err) {
  ...
  const handleDownloadSampleCsv = () => {
    const blob = new Blob([SAMPLE_TRANSACTION_CSV], { type: 'text/csv;charset=utf-8;' });
  ...
  const handleDownloadResultsCsv = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'fraudshield_prediction_results.csv';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
